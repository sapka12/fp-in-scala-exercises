...
object RNG {
  ...
  def doubleViaRand: Rand[Double] = map(int)(i => (1.0 + i + Int.MaxValue) / (2.0 * Int.MaxValue + 2))
  ...
  def sequence[A](fs: List[Rand[A]]): Rand[List[A]] = fs
    .foldRight[Rand[List[A]]](r => (List.empty[A], r))((rngA, rngList) =>
